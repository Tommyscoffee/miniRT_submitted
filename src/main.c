...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/05 02:57:55 by akihito           #+#    #+#             */
/*   Updated: 2022/12/05 02:57:59 by akihito          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_color_to_image(t_display *display, t_world *world)
{
	...
	while (y < HEIGHT)
	{
		to_screen_y = vector_3d_add(
				world->camera->screen_start_pos,
				vector_3d_mult(world->camera->screen_vertical_normal,
					world->camera->dy * (double)y));
		x = 0;
		while (x < WIDTH)
		{
			to_screen = vector_3d_add(to_screen_y,
					vector_3d_mult(world->camera->screen_horizon_normal,
